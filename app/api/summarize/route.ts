...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey)
      return NextResponse.json({ error: "API key not configured. Add ANTHROPIC_API_KEY to .env.local" }, { status: 500 });
...
${text}`,
    ...
    const points     = pointsRaw.split("\n").filter((p) => p.trim().startsWith("-")).map((p) => p.replace(/^-\s*/, "").trim()).filter(Boolean);
